 ...
 if(isset($_POST['thanhtoanngay']))
  {
    ...
    $value=$_POST['soluong'];
                ...
                $sql="select * from sanpham where MASANPHAM='".$mh."'";
                $query=mysqli_query($con,$sql);
                while($dong = mysqli_fetch_array($query)){
                    
                    $gia=$dong['GIASANPHAM'];
                    $sql_themct="insert into chitietdondathang (MASANPHAM,MADONDATHANG,SOLUONG,GIABAN) value($mh,$maDH,$value,$gia)";
                    mysqli_query($con,$sql_themct);
                    $slt_m=$dong['SOLUONGTON']-$value;
                    $slm_m=$dong['SOLUONGBAN']+$value;
                    $sql_up="update sanpham set SOLUONGTON='$slt_m',SOLUONGBAN='$slm_m' where MASANPHAM='$mh'";
